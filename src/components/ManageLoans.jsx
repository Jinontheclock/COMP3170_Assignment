...
function ManageLoans({ books, borrowedBooks, onBorrow }) {
  ...
  // Filter available books (books not in borrowedBooks) - optimized with Set
  const borrowedIsbnSet = useMemo(() => new Set(borrowedBooks.map(b => b.isbn13)), [borrowedBooks]);
  const availableBooks = useMemo(() => 
    books.filter(book => !borrowedIsbnSet.has(book.isbn13))
  , [books, borrowedIsbnSet]);
